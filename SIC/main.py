...
async def simple_detect(file: UploadFile = File(...),
                        model_name: str = Form('yolo-v8n-version1')):
    logger.info("Video Prediction is ongoing")
    current_time = datetime.datetime.now()
    time_str = current_time.strftime("%Y%m%d_%H%M%S")
    ...
    # Process each frame and collect in a list
    frames = []
    while True:
        ret, frame = cap.read()
        if not ret:
    ...
    output_path = os.path.join(output_dir, f"{time_str}.mp4")
    ...
    rand = random.randint(0, 100000)
    file_dir = os.path.join(output_dir, f"file_{time_str}_{rand}.mp4")
    logger.info(f"Output is saved in {file_dir}")
